...
impl fmt::Display for ApplicationError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            ...
            ApplicationError::EndTransactionError(ref s) => write!(f,"{}",s),
...
impl error::Error for ApplicationError {
    fn description(&self) -> &str {
        match *self {
            ...
            ApplicationError::EndTransactionError(_) => "An error occurred in the transaction termination process.",
    ...
    fn source(&self) -> Option<&(dyn error::Error + 'static)> {
        match *self {
            ...
            ApplicationError::EndTransactionError(_) => None,
...
impl From<SendError<(VecDeque<LegalMove>, Score, ZobristHash<u64>)>> for ApplicationError {
    fn from(err: SendError<(VecDeque<LegalMove>, Score, ZobristHash<u64>)>) -> ApplicationError {
        ApplicationError::TransactionCompleteCallbackSendError(err)
    }
}
